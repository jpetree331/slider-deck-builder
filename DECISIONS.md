...
## 2026-08-17 · Store · Slides keep the painter's honest format (01.jpg / 01.png)
Painters generate JPEG almost always; the 08-14 format-truth fix transcoded that to PNG, which is lossless-wrapping already-lossy pixels — 5× the bytes for zero quality gain (a 13-slide deck weighed 107 MB; its exports 102 MB). Slides now store the painter's native bytes with an honest extension (exotic formats like WebP still transcode to PNG so PowerPoint/img2pdf never meet them); position == filename holds, only the extension varies; `store.find_slide_image` is the one resolver. Image URLs became `/slides/{n}/image` (Content-Type carries the truth; `{n}.png` stays as a legacy alias). Existing PNG decks remain valid — repaints migrate them slide by slide.
